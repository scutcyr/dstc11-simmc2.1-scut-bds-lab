...
class LineByLineDataset(Dataset):
    def __init__(self, input_file, target_file, disambiguation_file, response_file, tokenizer: PreTrainedTokenizer, all_objects_meta, evaluation=False, model_type=None, use_OBJ=True):
...
def get_dataset(args, tokenizer, all_objects_meta, train=True):
    if train:
        dataset = LineByLineDataset(args.train_input_file, args.train_target_file, args.disambiguation_file, args.response_file, tokenizer, all_objects_meta, model_type=args.model_type, use_OBJ=not args.not_use_OBJ)
    else:
        dataset = LineByLineDataset(args.eval_input_file, args.eval_target_file, None, None, tokenizer, all_objects_meta, evaluation=True, model_type=args.model_type, use_OBJ=not args.not_use_OBJ)
...
class DisambiguationDataset(Dataset):
    '''用于歧义识别任务（DSTC-10 任务1，二分类）
       特别地，在本项目当中主要用于eval_model_disambiguation.py任务

    '''
    def __init__(self, prompts_from_file, disambiguation_file, tokenizer, model_type=None, use_OBJ=True):
...
class GenerationDataset(Dataset):
    ...
    '''
    def __init__(self, prompts_from_file, tokenizer, model_type=None, use_OBJ=True):
        """
