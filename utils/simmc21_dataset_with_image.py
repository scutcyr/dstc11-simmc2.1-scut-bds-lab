...
class LineByLineDatasetWithImage(Dataset):
    def __init__(self, 
                 input_file, 
                 target_file, 
                 disambiguation_file, 
        ...
        if image_path_file is not None:
            ...
            if image_dir is not None:
                ...
                if feature_extractor is not None:
                    self.feature_extractor = feature_extractor # FlavaFeatureExtractor, ...
        ...
        with open(input_file, encoding="utf-8") as f:
            for line in f.read().splitlines():
                if (len(line) > 0 and not line.isspace()):
                    ...
                    if not use_BF:
                        # 去掉 "=> Belief State : "
                        line = re.sub(r"=> Belief State : ", "", line)
        ...
        # 请注意检查tokenizer是否输出token_type_ids
        self.examples_token_type_ids = encode_text.token_type_ids
    ...
    def __getitem__(self, i):
        ...
        if self.do_extract_image_feature:
            ...
            if image == False:
                # 图像不存在，则返回全0的Tensor
                image_feature = torch.zeros([3, 224, 224], dtype=torch.float)
            else:
                # 图像存在
                image_feature = self.feature_extractor(image, return_tensors="pt")['pixel_values'][0] # torch.Size([3, 224, 224])
        ...
        if not self.evaluation:
            if self.do_extract_image_feature:
                return torch.tensor(self.examples[i], dtype=torch.long), torch.tensor(self.examples_attention_mask[i], dtype=torch.long), \
                        ...
                        self.response[i], self.disam[i], image_feature, torch.tensor(self.examples_token_type_ids[i], dtype=torch.long)
        ...
        if self.do_extract_image_feature:
            # 读取图像特征
            return torch.tensor(self.examples[i], dtype=torch.long), torch.tensor(self.examples_attention_mask[i], dtype=torch.long), \
                self.generation[i], self.boxes[i], self.misc[i], self.nocoref[i], self.disam[i], image_feature, torch.tensor(self.examples_token_type_ids[i], dtype=torch.long)
        else:
            return torch.tensor(self.examples[i], dtype=torch.long), torch.tensor(self.examples_attention_mask[i], dtype=torch.long), \
                self.generation[i], self.boxes[i], self.misc[i], self.nocoref[i], self.disam[i]


def get_dataset_with_image(args, tokenizer, all_objects_meta, train=True, feature_extractor=None):
    if train:
        dataset = LineByLineDatasetWithImage(input_file=args.train_input_file, 
                                             ...
                                             use_BF=not args.not_use_BF)
    else:
        dataset = LineByLineDatasetWithImage(input_file=args.eval_input_file, 
