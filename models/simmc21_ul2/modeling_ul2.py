...
from .configuration_ul2 import T5Config
...
class T5PreTrainedModel(PreTrainedModel):
    ...
    def _init_weights(self, module):
        ...
        elif isinstance(module, (T5Model, MultiTaskUL2ForConditionalGeneration, T5EncoderModel)):
...
"""
PyTorch MultiTaskUL2ForConditionalGeneration Class for DSTC-11 SIMMC 2.1
Changed Based on MultiTaskUL2ForConditionalGeneration
Updated by Yirong Chen 
Used for [SIMMC 2.1](https://github.com/facebookresearch/simmc2)
Mail: [eeyirongchen@mail.scut.edu.cn](mailto:eeyirongchen@mail.scut.edu.cn)
Date: 2022/08/15
...
class MultiTaskUL2ForConditionalGeneration(T5PreTrainedModel):
    _keys_to_ignore_on_load_missing = [
        r"encoder.embed_tokens.weight",
        r"decoder.embed_tokens.weight",
        r"lm_head.weight",
    ...
    def __init__(self, config: T5Config):
        ...
        # Add module here
        # Updated by Yirong Chen
        self.box_embedding = BoxEmbedding(config.d_model, dropout_rate=config.dropout_rate)
        self.nocoref_head = NoCorefHead(config.d_model, dropout_rate=config.dropout_rate)
        self.fashion_enc_head = FashionEncoderHead(config.d_model, dropout_rate=config.dropout_rate)
        self.furniture_enc_head = FurnitureEncoderHead(config.d_model, dropout_rate=config.dropout_rate)
        self.disambiguation_head = DisambiguationHead(config.d_model, dropout_rate=config.dropout_rate)
    ...
    def forward(
        ...
        ```python
        >>> from transformers import T5Tokenizer, MultiTaskUL2ForConditionalGeneration

        >>> tokenizer = T5Tokenizer.from_pretrained("t5-small")
        >>> model = MultiTaskUL2ForConditionalGeneration.from_pretrained("t5-small")
        ...
        # For Disambiguation
        # UL2的输入： ['[', 'S', '2', 'S', ']' , '<DISAM>', ... , '</s>']
        disambiguation_logits = self.disambiguation_head(enc_last_state[:, 5, :]) # bs, d_model --> bs, 2
