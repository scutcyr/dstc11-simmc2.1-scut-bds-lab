...
class FlavaModelOutput(ModelOutput):
    ...
    loss_disamb: Optional[torch.FloatTensor] = None
    loss_coref: Optional[torch.FloatTensor] = None
    logits_disamb: Optional[torch.FloatTensor] = None
    logits_coref: Optional[torch.FloatTensor] = None
...
@dataclass
class FlavaModelOutputForSIMMC(ModelOutput):
    """
    Output from FlavaModel containing embeddings and outputs from individual encoders.
...
class FlavaTextEmbeddings(nn.Module):
    ...
    def __init__(self, config):
        ...
        # 新增
        self.box_embedding = BoxEmbedding(config.hidden_size, dropout_rate=config.hidden_dropout_prob)
    ...
    def forward(
        ...
        if boxes is not None and misc is not None: # 考虑box_embedding
            # 增加box_embedding
            batch_size = len(misc)
...
class FlavaTextModel(FlavaPreTrainedModel):
    ...
    def forward(
        ...
        embedding_output = self.embeddings(
            ...
            boxes=boxes,  # Updated by Yirong Chen
            misc=misc,  # Updated by Yirong Chen
        )
...
"""
PyTorch MultiTaskFlavaModel Class for DSTC-11 SIMMC 2.1
Changed Based on FlavaModel
Updated by Yirong Chen 
...
class MultiTaskFlavaModel(FlavaPreTrainedModel):
    config_class = FlavaConfig
    ...
    def __init__(self, config: FlavaConfig):
        ...
        # Add module here
        # Updated by Yirong Chen
        #self.box_embedding = BoxEmbedding(self.mm_hidden_size, dropout_rate=0.0) # 直接加到text_model.embeddings里面了
        self.nocoref_head = NoCorefHead(self.mm_hidden_size, dropout_rate=multimodal_config.attention_probs_dropout_prob)
        self.fashion_enc_head = FashionEncoderHead(self.mm_hidden_size, dropout_rate=multimodal_config.attention_probs_dropout_prob)
        self.furniture_enc_head = FurnitureEncoderHead(self.mm_hidden_size, dropout_rate=multimodal_config.attention_probs_dropout_prob)
        self.disambiguation_head = DisambiguationHead(self.mm_hidden_size, dropout_rate=multimodal_config.attention_probs_dropout_prob)


        #self.disamb_linear = nn.Linear(self.mm_hidden_size, 2) # 用于判断每个object是否为歧义候选disambiguation candidate, Updated by Yirong Chen
        #self.coref_linear = nn.Linear(self.mm_hidden_size, 2) # 用于判断每个object是否为MM-Coref object
    ...
    def get_image_encoder(self):
        '''新增 by Yirong Chen
        '''
        return self.image_model.encoder

    def get_multimodal_encoder(self):
        '''新增 by Yirong Chen
        '''
        return self.multimodal_model.encoder
    ...
    def resize_token_embeddings(self, new_num_tokens: int) -> nn.Embedding:
        '''新增 by Yirong Chen
        '''
        #new_embeddings = self.text_model.resize_token_embeddings(new_num_tokens)
        new_embeddings = super().resize_token_embeddings(new_num_tokens)
    ...
    def forward(
        ...
        pixel_values: Optional[torch.FloatTensor] = None, # (batch_size, 3, 224, 224)
        attention_mask: Optional[torch.Tensor] = None,
        token_type_ids: Optional[torch.Tensor] = None,
        bool_masked_pos: Optional[torch.Tensor] = None,
        position_ids: Optional[torch.LongTensor] = None,
        image_attention_mask: Optional[torch.Tensor] = None,
        disamb_label: Optional[torch.LongTensor] = None,
        coref_label: Optional[torch.LongTensor] = None,
    ...
    ) -> Union[Tuple, FlavaModelOutputForSIMMC]:
        r"""
        Returns:
        ...
        >>> url = "http://images.cocodataset.org/val2017/000000039769.jpg"
        >>> image = Image.open(requests.get(url, stream=True).raw)
        >>> image = image.convert("RGB") # 如果是.png图像，需要先转换为RBG三通道
        ...
        if pixel_values is not None:
            ...
            # image_embeddings: torch.Size([batch_size, 197, 768]) # 197=1+14*14
        ...
        if input_ids is not None:
            ...
            # image_embeddings: torch.Size([batch_size, max_length, 768])
            # Note that these states don't use final layernorm in the transformer model
        ...
        if image_mm_projection is not None and text_mm_projection is not None and not skip_multimodal_encoder:
            ...
            multimodal_output = self.multimodal_model(multimodal_input, return_dict=True)
            multimodal_embeddings = multimodal_output[0]
            # [CLS,M] [CLS,I] [Patch_0] ... [Patch_195] [CLS,T] [Token_1] ... [Token_N]
            # multimodal_embeddings: torch.Size([batch_size, 1 + image_embeddings.size()[1] + max_length, 768])
            text_shift_length = 1 + image_embeddings.size()[1] # [CLS,M] [CLS,I] [Patch_0] ... [Patch_195]的长度
        
        if do_retrieval: # 训练阶段
            # For Biencoder
            response_vec = self.text_model(input_ids=response, attention_mask=response_attention_mask)[0][:, 0, :] # bs, dim
            # 利用[CLS,M]位置的隐藏层输出计算
            context_vec = multimodal_embeddings[:, 0, :] # bs, dim
            ...
            retrieval_loss = torch.tensor(0).to(input_ids.device)
        
        # For Disambiguation
        # 利用<DISAM>位置的隐藏层输出计算
        disambiguation_logits = self.disambiguation_head(multimodal_embeddings[:, text_shift_length+1, :]) # bs, d_model --> bs, 2
        if disambiguation_labels is not None: # 训练阶段
            ...
            disam_loss = torch.tensor(0).to(input_ids.device)

        if (nocoref is not None) and (isinstance(nocoref[0],tuple)): # 训练阶段
            # [(position, label), (position, label), (position, label),...]
            nocoref_logits = torch.stack([self.nocoref_head(multimodal_embeddings[b_idx][nocoref[b_idx][0]+text_shift_length]) for b_idx in range(batch_size) ])
            ...
            nocoref_loss = torch.tensor(0).to(input_ids.device)
        else:
            nocoref_logits = None
            nocoref_loss = torch.tensor(0).to(input_ids.device)
        ...
        if misc is not None and "coref_label" in misc[0][0]:
            ...
            for b_idx in range(batch_size):  # in a batch
                ...
                for obj_idx in range(len(misc[b_idx])):
                    ...
                    if obj_idx == 0:
                        hidden_concat = torch.reshape(multimodal_embeddings[b_idx][pos+text_shift_length:pos+text_shift_length+2], (1,-1))
                    else:
                        hidden_concat = torch.cat([hidden_concat, torch.reshape(multimodal_embeddings[b_idx][pos+text_shift_length:pos+text_shift_length+2], (1,-1))], dim=0)
                    # hidden_concat = torch.reshape(multimodal_embeddings[b_idx][pos:pos+2], (-1,))  # (2*d_model)  -> 
                """
                # 为了解决报错：
            ...
            """ eval and test
                不计算loss
            """
            misc_loss = torch.tensor(0).to(input_ids.device)
            enc_head_results = []
            ...
            for b_idx in range(batch_size):
                ...
                objs_pos = [misc[b_idx][obj_idx]['pos']+text_shift_length for obj_idx in range(len(misc[b_idx]))]
        ...
        if use_pooled_output:
            cls_last_hidden_state = multimodal_output.pooled_output
        else:
            cls_last_hidden_state = multimodal_output.last_hidden_state[:, 0, :]
        
        logits_disamb = self.disamb_linear(cls_last_hidden_state) # (bs, seqlen, d_model) last_hidden_state[0]
        logits_coref = self.coref_linear(cls_last_hidden_state)

        ce_loss_fct = CrossEntropyLoss()

        if disamb_label is not None:
            loss_disamb = ce_loss_fct(logits_disamb, disamb_label) # 判断当前object_id是否为歧义候选
        else:
            loss_disamb = torch.tensor([0]).to(input_ids.device)
        if coref_label is not None:
            loss_coref = ce_loss_fct(logits_coref, coref_label) # 判断当前object_id是否为多模态共指消解
        else:
            loss_coref = torch.tensor([0]).to(input_ids.device)

        loss = loss_disamb + loss_coref
        '''
        masked_lm_loss = torch.tensor(0).to(input_ids.device)
        ...
        return FlavaModelOutputForSIMMC(
            loss=loss,
...
class FlavaModel(FlavaPreTrainedModel):
    ...
    def forward(
        ...
        return FlavaModelOutput(
            loss=loss,
            loss_disamb=loss_disamb,
            loss_coref=loss_coref,
            logits_disamb=logits_disamb,
            logits_coref=logits_coref,
            image_embeddings=image_embeddings,
            image_output=image_output,
            text_embeddings=text_embeddings,
            text_output=text_output,
