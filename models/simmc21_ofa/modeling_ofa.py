...
class OFAEncoder(OFAPreTrainedModel):
    ...
    def __init__(self, config: OFAConfig, embed_tokens: Optional[nn.Embedding] = None):
        ...
        # 新增
        self.box_embedding = BoxEmbedding(embed_dim, dropout_rate=0.0)
    ...
    def forward_embedding(
        ...
        if boxes is not None and misc is not None: # 考虑box_embedding
            ...
            for b_idx in range(batch_size):  # in a batch
                ...
                for obj_idx in range(len(misc[b_idx])):
                    ...
                    x[b_idx][pos] += box_embedded[obj_idx]
        ...
        if image_embed is not None:
            ...
            if self.type_embedding is not None:
                image_x += self.type_embedding(input_ids.new_ones(image_x.size()[:2])) # [batch_size, sample_patch_num]的全1向量
    ...
    def forward(
        ...
        sample_patch_num: Optional[int] = 256,   # 16*16 See A.2 Pretraining Details
        boxes: Optional[List[List[List[float]]]] = None, # 新增 考虑batch_size, # batch, num_obj_per_line, 6
        misc: Optional[List[List[Dict]]] = None, # 新增 [ [ {pos, coref_label, misc_labels(dict), is_fashion}, ... ], ...], 考虑batch_size, # batch, num_obj_per_line, dict
    ):
        r"""
        ...
        x, encoder_embedding = self.forward_embedding(
            ...
            pos_embed, image_pos_embed, image_pos_embed_2, boxes, misc
        )
...
"""
PyTorch OFAModelForSIMMCGeneration Class for DSTC-11 SIMMC 2.1
Changed Based on OFAModel
...
class OFAModelForSIMMCGeneration(OFAPreTrainedModel):
    r"""
    ...
    def set_output_embeddings(self, new_embeddings):
        # Add by Yirong Chen
        self.decoder.output_projection = new_embeddings
    ...
    def forward(
            ...
            sample_patch_num=256,  # # 16*16 See A.2 Pretraining Details
        ...
        if labels is not None:
            lm_logits = decoder_outputs.last_hidden_state # 已经经过LM Head
            ...
            masked_lm_loss = torch.tensor(0.0).to(encoder_hidden_states.device)
...
"""
PyTorch MultiTaskOFAModelForConditionalGeneration Class for DSTC-11 SIMMC 2.1
Changed Based on OFAModel
Updated by Yirong Chen 
Used for [SIMMC 2.1](https://github.com/facebookresearch/simmc2)
Mail: [eeyirongchen@mail.scut.edu.cn](mailto:eeyirongchen@mail.scut.edu.cn)
...
class MultiTaskOFAModelForConditionalGeneration(OFAPreTrainedModel):
    r"""
    The OFA model built with an encoder and a decoder only, with several classification head.
    ...
    def forward(
        ...
        elif encoder_outputs is not None and not isinstance(encoder_outputs, ModelOutput):
            batch_size = len(encoder_outputs[0])
        elif encoder_outputs is not None:
            batch_size = len(encoder_outputs.last_hidden_state)
        elif token_embeddings is not None:
        ...
        # 利用encoder_hidden_states计算
        # [patch0] ... [patch255] <s> <DISAM> ...

        # 检索任务
        retrieval_loss = torch.tensor(0.0).to(encoder_hidden_states.device)

        # For Disambiguation
        disambiguation_logits = self.disambiguation_head(encoder_hidden_states[:, 1+sample_patch_num, :]) # bs, d_model --> bs, 2
        ...
        if disambiguation_labels is not None: # 训练阶段
            ...
            disam_loss = torch.tensor(0.0).to(encoder_hidden_states.device)

        # For No Coref
        if (nocoref is not None) and (isinstance(nocoref[0],tuple)): # 训练阶段
            # [(position, label), (position, label), (position, label),...]
            nocoref_logits = torch.stack([self.nocoref_head(encoder_hidden_states[b_idx][nocoref[b_idx][0]+sample_patch_num]) for b_idx in range(batch_size) ])
            ...
            nocoref_logits = torch.stack([self.nocoref_head(encoder_hidden_states[b_idx][nocoref[b_idx]+sample_patch_num]) for b_idx in range(batch_size)])
            #is_nocoref = nocoref_logits.argmax(dim=1).bool()
            nocoref_loss = torch.tensor(0.0).to(encoder_hidden_states.device)
        else:
            nocoref_logits = None
            nocoref_loss = torch.tensor(0.0).to(encoder_hidden_states.device)
        ...
        if misc is not None and "coref_label" in misc[0][0]:
            ...
            for b_idx in range(batch_size):  # in a batch
                ...
                for obj_idx in range(len(misc[b_idx])):
                    pos = misc[b_idx][obj_idx]['pos']+sample_patch_num
                    # hidden_concat: (num_obj, 2*model)
                    if obj_idx == 0:
                        hidden_concat = torch.reshape(encoder_hidden_states[b_idx][pos:pos+2], (1,-1))
                    else:
                        hidden_concat = torch.cat([hidden_concat, torch.reshape(encoder_hidden_states[b_idx][pos:pos+2], (1,-1))], dim=0)
                    # hidden_concat = torch.reshape(encoder_hidden_states[b_idx][pos:pos+2], (-1,))  # (2*d_model)  -> 
                """
                # 为了解决报错：
            ...
            """
            enc_head_results = []
            misc_loss = torch.tensor(0).to(encoder_hidden_states.device)
