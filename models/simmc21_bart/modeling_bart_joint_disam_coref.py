...
from .modeling_bart_outputs import Seq2SeqLMOutputForSIMMC, Seq2SeqLMOutputForSIMMCJointDisamCoref
...
# 2022.09.09 新增
class JointDisambAndCorefHead(nn.Module):
    """ Copy from DSTC-10 SIMMC2.0 Team 4
        Updated by Yirong Chen on 2022/08/04
        Reference: 
            https://github.com/KAIST-AILab/DSTC10-SIMMC/blob/dstc10-simmc2-v1/scripts/run_bart_multi_task.py
    ...
    """
    def __init__(self, hidden_dim, dropout_rate=0.0):
        super(JointDisambAndCorefHead, self).__init__()
        self.joint_disamb_coref_linear = nn.Linear(hidden_dim, 3)  # 三分类，0：无歧义且无共指；1：有歧义无共指；2：无歧义有共指
        #self.dropout = nn.Dropout(dropout_rate) # 新增
    def forward(self, x):
        #return self.dropout(self.disamb_linear(x))
        return self.joint_disamb_coref_linear(x)


class UserActHead(nn.Module):
    """ Copy from DSTC-10 SIMMC2.0 Team 4
        Updated by Yirong Chen on 2022/09/09
        Reference: 
            https://github.com/KAIST-AILab/DSTC10-SIMMC/blob/dstc10-simmc2-v1/scripts/run_bart_multi_task.py
            https://aclanthology.org/2022.findings-naacl.61/
        识别当前对话的用户的意图
    """
    def __init__(self, hidden_dim, dropout_rate=0.0):
        super(UserActHead, self).__init__()
        self.user_act_linear = nn.Linear(hidden_dim, 7)  # REQUEST:GET、REQUEST:ADD_TO_CART、REQUEST:COMPARE、INFORM:GET、INFORM:REFINE、INFORM:DISAMBIGUATE、ASK:GET
        #self.dropout = nn.Dropout(dropout_rate) # 新增
    def forward(self, x):
        #return self.dropout(self.disamb_linear(x))
        return self.user_act_linear(x)


class SystemActHead(nn.Module):
    """ Copy from DSTC-10 SIMMC2.0 Team 4
        Updated by Yirong Chen on 2022/09/09
        Reference: 
            https://github.com/KAIST-AILab/DSTC10-SIMMC/blob/dstc10-simmc2-v1/scripts/run_bart_multi_task.py
    ...
    """
    def __init__(self, hidden_dim, dropout_rate=0.0):
        super(SystemActHead, self).__init__()
        self.system_act_linear = nn.Linear(hidden_dim, 4)  # INFORM:GET、INFORM:COMPARE、REQUEST:DISAMBIGUATE、CONFIRM:ADD_TO_CART
        #self.dropout = nn.Dropout(dropout_rate) # 新增
    def forward(self, x):
        #return self.dropout(self.disamb_linear(x))
        return self.system_act_linear(x)



class JointFashionEncoderHead(nn.Module):
    """ Copy from DSTC-10 SIMMC2.0 Team 4
        Updated by Yirong Chen on 2022/09/09
        Reference: 
            https://github.com/KAIST-AILab/DSTC10-SIMMC/blob/dstc10-simmc2-v1/scripts/run_bart_multi_task.py
            https://aclanthology.org/2022.findings-naacl.61/
        Used for JointFashionEncoderHead
    """
    def __init__(self, hidden_dim, dropout_rate=0.0):
        super(JointFashionEncoderHead, self).__init__()
        ...
        self.disamb_coref_linear = nn.Linear(2*hidden_dim, 3)  # 三分类，0：无歧义且无共指；1：有歧义无共指；2：无歧义有共指
    ...
    def forward(self, concat_vector):
        ...
        disam_coref = self.disamb_coref_linear(aggregated)
        ...
        return disam_coref, size, available_sizes, brand, color, pattern, sleeve_length, asset_type, type_, \
               price, customer_review

class JointFurnitureEncoderHead(nn.Module):
    """ Copy from DSTC-10 SIMMC2.0 Team 4
        Updated by Yirong Chen on 2022/09/09
        Reference: 
            https://github.com/KAIST-AILab/DSTC10-SIMMC/blob/dstc10-simmc2-v1/scripts/run_bart_multi_task.py
            https://aclanthology.org/2022.findings-naacl.61/
        Used for JointFurnitureEncoderHead
    """
    def __init__(self, hidden_dim, dropout_rate=0.0):
        super(JointFurnitureEncoderHead, self).__init__()
        self.aggregator = nn.Linear(2*hidden_dim, 2*hidden_dim)
        #self.disamb_linear = nn.Linear(2*hidden_dim, 2) # 用于判断每个object是否为歧义候选disambiguation candidate, Updated by Yirong Chen
        #self.coref_linear = nn.Linear(2*hidden_dim, 2) # 用于判断每个object是否为MM-Coref object
        self.disamb_coref_linear = nn.Linear(2*hidden_dim, 3)  # 三分类，0：无歧义且无共指；1：有歧义无共指；2：无歧义有共指
    ...
    def forward(self, concat_vector):
        ...
        price = self.price_linear(aggregated)
        customer_review = self.customer_review_linear(aggregated)
        return disam_coref, brand, color, materials, type_, price, customer_review
...
class MultiTaskBartForConditionalGenerationJointDisambCoref(BartPretrainedModel):
    base_model_prefix = "model"
    ...
    def __init__(self, config: BartConfig):
        ...
        self.user_act_head = UserActHead(config.d_model, dropout_rate=config.dropout) # 用户意图识别
        self.system_act_head = SystemActHead(config.d_model, dropout_rate=config.dropout) # 系统回复意图预测
        self.joint_disamb_and_coref_head = JointDisambAndCorefHead(config.d_model, dropout_rate=config.dropout)
        self.fashion_enc_head = JointFashionEncoderHead(config.d_model, dropout_rate=config.dropout)
        self.furniture_enc_head = JointFurnitureEncoderHead(config.d_model, dropout_rate=config.dropout)
    ...
    def forward(
        ...
        disam_and_coref_labels: Optional[torch.LongTensor] = None, # 新增，替代disambiguation_labels
        user_act_labels: Optional[torch.LongTensor] = None, # 新增，用户意图标签
        system_act_labels: Optional[torch.LongTensor] = None, # 新增，系统意图标签
        ...
        alpha_user_act_loss: Optional[float] = 1.0, # 新增，识别用户意图的损失占比
        alpha_system_act_loss: Optional[float] = 1.0, # 新增，预测系统意图的损失占比
        alpha_disamb_and_coref_loss: Optional[float] = 1.0, # 新增，预测当前对话是否存在歧义候选或者多模态共指现象的损失占比
        alpha_disam_and_coref_candi_loss: Optional[float] = 1.0, # 新增，判断object是否属于歧义候选或者多模态共指的损失占比
        use_focal_loss: Optional[bool] = False,
        focal_loss_gamma: Optional[float] = 5.0,
    ) -> Union[Tuple, Seq2SeqLMOutputForSIMMCJointDisamCoref]:
        r"""
        ...
        # 用户意图识别任务，7分类
        # 利用开始位置<s>的隐藏层输出作为语义向量
        user_act_logits = self.user_act_head(enc_last_state[:, 0, :]) # bs, d_model --> bs, 7
        if user_act_labels is not None: # 训练阶段
            user_act_loss = ce_loss_fct(user_act_logits, user_act_labels.view(-1))
        else: # 验证或测试阶段或该任务不参与训练
            user_act_loss = torch.tensor(0.0).to(user_act_logits.device)

        # 系统意图预测任务，4分类
        # 利用开始位置<s>的隐藏层输出作为语义向量
        system_act_logits = self.system_act_head(enc_last_state[:, 0, :]) # bs, d_model --> bs, 4
        if system_act_labels is not None: # 训练阶段
            system_act_loss = ce_loss_fct(system_act_logits, system_act_labels.view(-1))
        else: # 验证或测试阶段或该任务不参与训练
           system_act_loss = torch.tensor(0.0).to(system_act_logits.device)

        # 联合判断当前是否存在歧义或者多模态共指任务，3分类
        # 利用开始位置<DISAM>的隐藏层输出作为语义向量
        disamb_and_coref_logits = self.joint_disamb_and_coref_head(enc_last_state[:, 1, :]) # bs, d_model --> bs, 3
        if disam_and_coref_labels is not None: # 训练阶段
            disamb_and_coref_loss = ce_loss_fct(disamb_and_coref_logits, disam_and_coref_labels.view(-1))
        else: # 验证或测试阶段或该任务不参与训练
            disamb_and_coref_loss = torch.tensor(0.0).to(disamb_and_coref_logits.device)

        # 属性识别与歧义候选识别、多模态共指消解
        misc_loss = 0 # 属性预测
        disam_and_coref_candi_loss = 0 # 歧义候选或者多模态共指
        if misc is not None and "disam_and_coref_label" in misc[0][0]:
            # 思考这里的判断条件
            """ train 阶段，计算loss
            """
            # 不能并行
            ...
            num_fashions = 0
            num_furnitures = 0
            fashions_misc_loss = 0
            furnitures_misc_loss = 0
            fashions_disam_and_coref_candi_loss = 0
            furnitures_disam_and_coref_candi_loss = 0
            for b_idx in range(batch_size):  # in a batch
                ...
                disam_and_coref_label = [misc[b_idx][obj_idx]['disam_and_coref_label'] for obj_idx in range(len(misc[b_idx]))]  # (num_obj)  0 or 1 or 2
                if is_fashion:
                    num_fashions +=1
                ...
                """
                fashion_disam_and_coref, fashion_size, fashion_available_sizes, fashion_brand, fashion_color, fashion_pattern, fashion_sleeve_length, \
                ...
                furniture_disam_and_coref, furniture_brand, furniture_color, furniture_materials, furniture_type_, furniture_price, furniture_customer_review = self.furniture_enc_head(hidden_concat)  # (num_obj, num_logits)
                
                if is_fashion:
                    enc_head_results.append((fashion_disam_and_coref, fashion_size, fashion_available_sizes, fashion_brand, fashion_color, fashion_pattern, fashion_sleeve_length, \
                    fashion_asset_type, fashion_type_, fashion_price, fashion_customer_review))
                    fashions_disam_and_coref_candi_loss += ce_loss_fct(fashion_disam_and_coref, torch.tensor(disam_and_coref_label, dtype=torch.long).to(input_ids.device))
                    fashions_misc_loss += (ce_loss_fct(fashion_size, torch.tensor(fashion_size_label, dtype=torch.long).to(input_ids.device)) + \
                    ...
                    enc_head_results.append((furniture_disam_and_coref, furniture_brand, furniture_color, furniture_materials, furniture_type_, furniture_price, furniture_customer_review))
                    furnitures_disam_and_coref_candi_loss += ce_loss_fct(furniture_disam_and_coref, torch.tensor(disam_and_coref_label, dtype=torch.long).to(input_ids.device))
                    furnitures_misc_loss += (ce_loss_fct(furniture_brand, torch.tensor(furniture_brand_label, dtype=torch.long).to(input_ids.device)) + \
            ...
            #print("fashions_misc_loss=", fashions_misc_loss)    
            #print("furnitures_misc_loss=", furnitures_misc_loss)
            if num_fashions != 0 and num_furnitures != 0:
                disam_and_coref_candi_loss = fashions_disam_and_coref_candi_loss/num_fashions + furnitures_disam_and_coref_candi_loss/num_furnitures
                misc_loss = fashions_misc_loss/num_fashions + furnitures_misc_loss/num_furnitures
            else:
                disam_and_coref_candi_loss = (fashions_disam_and_coref_candi_loss + furnitures_disam_and_coref_candi_loss)/batch_size
                misc_loss = (fashions_misc_loss + furnitures_misc_loss)/batch_size
        ...
        loss = alpha_masked_lm_loss*masked_lm_loss + alpha_retrieval_loss*retrieval_loss + alpha_user_act_loss*user_act_loss + alpha_system_act_loss*system_act_loss + \
               + alpha_disamb_and_coref_loss*disamb_and_coref_loss + alpha_disam_and_coref_candi_loss*disam_and_coref_candi_loss + alpha_misc_loss*misc_loss


        if not return_dict:
            return [loss,masked_lm_loss,retrieval_loss,user_act_loss,system_act_loss,disamb_and_coref_loss,disam_and_coref_candi_loss,misc_loss]
        ...
        return Seq2SeqLMOutputForSIMMCJointDisamCoref(
            loss=loss,
            masked_lm_loss=masked_lm_loss,
            retrieval_loss=retrieval_loss,
            user_act_loss=user_act_loss,
            system_act_loss=system_act_loss,
            disamb_and_coref_loss=disamb_and_coref_loss,
            disam_and_coref_candi_loss=disam_and_coref_candi_loss,
            misc_loss=misc_loss,
            logits=lm_logits,
            user_act_logits=user_act_logits,
            system_act_logits=system_act_logits,
            disamb_and_coref_logits=disamb_and_coref_logits,
