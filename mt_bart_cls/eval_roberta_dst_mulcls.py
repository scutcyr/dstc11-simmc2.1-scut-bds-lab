...
def main():
    ...
    tokenizer = RobertaTokenizerFast.from_pretrained(args.test_model_dir)
    model = RobertaForSIMMC.from_pretrained(args.test_model_dir)
    ...
    for i, batch in enumerate(tqdm(decode_dataloader, desc='Decoding')):
        ...
        with torch.no_grad():
            ## 加上 box embedding
            inputs_embeds = model.roberta.embeddings(enc_input)
            ...
            encoder_outputs = model.roberta(inputs_embeds=inputs_embeds, attention_mask=enc_input_attention, return_dict=True)  # check this line
