...
class MultiTaskBartForConditionalGenerationWithDisamb(BartPretrainedModel):
    ...
    def __init__(self, config: BartConfig):
        ...
        self.dst_head = DSTHead(config.d_model)
    ...
    def forward(
        ...
        if dst is not None:
            # pdb.set_trace()
            dst_id_state = torch.stack([enc_last_state[b_idx, dst[b_idx]["pos"], :] for b_idx in range(batch_size)])
        ...
        loss = masked_lm_loss + 0.1 * nocoref_loss + 0.1 * misc_loss + 0.1 * disam_loss + 0.4 * retrieval_loss + 0.4 * dst_loss

        # pdb.set_trace()
        if not return_dict:
            return [loss, masked_lm_loss, nocoref_loss, misc_loss, disam_loss, retrieval_loss, dst_loss]
