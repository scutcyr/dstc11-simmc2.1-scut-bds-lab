...
TEMPLATE_PREDICT_USE_OBJVEC = "{context} {objvec} {sys_state} {START_BELIEF_STATE} "
TEMPLATE_PREDICT_OBJVEC_FIRST = "{objvec} {context} {sys_state} {START_BELIEF_STATE} "
TEMPLATE_TARGET = "{belief_state} {END_BELIEF_STATE} {response} {END_OF_SENTENCE}"

'''
prompt_api = api.PromptAPI(dial_split="train",
                           data_dir="~/projects/19_dstc11/simmc2.1_solutions/work/simmc2.1-scut-bds-lab/data", 
    ...
    for turn_idx, turn in enumerate(dialog[FIELDNAME_DIALOG]):
        ...
        if not revert:
            ...
            predict = TEMPLATE_PREDICT_OBJVEC_FIRST.format(
                ...
                context=context,
                sys_state=sys_state_merge,
                START_BELIEF_STAT=START_BELIEF_STATE
            )

        if with_target:
            target = TEMPLATE_TARGET.format(
                belief_state=str_belief_state,
                END_BELIEF_STATE=END_BELIEF_STATE,
        ...
        f_out.write("%s\t%s\t%s\t%s\t%s\t%s\n" % (predict, target, json.dumps(obj_boxes), 
