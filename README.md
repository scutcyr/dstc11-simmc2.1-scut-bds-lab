...
```bash
cp -rf ~/simmc2/data ~/dstc11-simmc2.1-iflytek/
cd ~/dstc11-simmc2.1-iflytek/data
ls # list the file
```

The files in ```~/dstc11-simmc2.1-iflytek/data```` are shown as follow:
```bash
...
IFLYTEK_PREPOCESS.md                simmc2.1_dials_dstc11_mini.json     simmc2_scene_images_dstc10_teststd.zip
...
* Change the ```INPUT_DIR=~/dstc11-simmc2.1-iflytek/data``` and ```WORK_DIR=~/dstc11-simmc2.1-iflytek``` to the actual path you specified; 
* Then run the dataset preprocessing scripts:
```bash
cd ~/dstc11-simmc2.1-iflytek/scripts
...
After preprocessing the dataset, you can find the preprocessed file in ```~/dstc11-simmc2.1-iflytek/data_convert```.
...
```bash
INIT_DATA_DIR=~/dstc11-simmc2.1-iflytek/data
PREPROCESS_DATA_DIR=~/dstc11-simmc2.1-iflytek/data_convert
